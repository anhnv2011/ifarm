...
class CreateSensor: BaseViewController, MKMapViewDelegate, CLLocationManagerDelegate, UITextFieldDelegate {
    ...
    var fromLocation: CLLocation? // lưu toạ độ của một địa điểm
    let locationManager = CLLocationManager()//NSLocationWhenInUseUsageDescription
    //class này là một lớp rất quan trọng cung cấp các tham số để cấu hình phục vụ cho việc cập nhật toạ độ thiết bị...
    
    var farmSelected : Farm?
    
    //----------------------------------------------------------------------------------------
    var overlay : MKOverlay? //-- vẽ đường đi
    var direction:MKDirections? //-- lây ra tuyến đường
    ...
    override func viewDidLoad() {
        ...
        geoCoder = CLGeocoder()
        geoCoder.geocodeAddressString("Hai Ba Trung Ha Noi") { (placemarks, error) in
    ...
    func mapView(mapView: MKMapView, viewForAnnotation annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if let annotation = annotation as? LocationAnnotation{
            ...
            {
                //dưới này sẽ là các sensors xung quanh
    ...
    //-- show len response -- trả về chi tiết các đường đi
